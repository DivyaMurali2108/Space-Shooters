...
function draw() {
  ...
  if(gameState === play) {
   ...
   // gameover = addImage(gameover);
    ...
    text("thank you from DivyaMurali",canvas.width/2-400, canvas.height/2);
  ...
  if(gameState === instruction) {
    ...
    textSize(50);
    fill("red");
    text("A Game By DivyaMurali",canvas.width/2-100,canvas.width/2-80);
    text("------SPACE SHOOTERS------",canvas.width/2-300,canvas.height/2-300);
    text("Wanna hear song while playing?,Click N!",canvas.width/2-360,canvas.height/2+110);
    stroke("yellow");
    fill("yellow");
    textSize(35);
